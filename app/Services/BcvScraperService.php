                ...
                if ($currencyNode) {
                    ...
                    if ($parentDiv) {
                        ...
                        if ($rateDiv) {
                            ...
                            if (is_numeric($rate)) {
                                $rates[$currencyCode] = floatval($rate);
                                Log::info("Tasa {$currencyCode} encontrada: {$rate}");
                            }
                        }
                    }
                }
    ...
    protected function saveRates(array $rates): void
    {
        try {
            ...
            foreach ($rates as $currencyCode => $rate) {
                ...
                Log::info("Nueva tasa guardada para {$currencyCode}: {$rate}");
            }

            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
            Log::error("Error al guardar tasas: " . $e->getMessage());
            throw $e;
        }
    }
